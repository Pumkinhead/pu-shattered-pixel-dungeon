...
import com.pumkinheadme.voicedshatteredpixeldungeon.windows.WndError;
import com.pumkinheadme.voicedshatteredpixeldungeon.windows.WndHardNotification;
...
public class WelcomeScene extends PixelScene {
	...
	private static final int LATEST_UPDATE = VoicedShatteredPixelDungeon.v0_1_0;
	...
	public void create() {
		...
		if (!triedCleaningTemp && FileUtils.cleanTempFiles()){
			add(new WndHardNotification(Icons.get(Icons.WARNING),
				...
				public void hide() {
					...
					VoicedShatteredPixelDungeon.resetScene();
				}
			});
			return;
		}

		if (VoicedShatteredPixelDungeon.versionCode == previousVersion && !SPDSettings.intro()) {
		...
		StyledButton okay = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(this, "continue")){
			...
			protected void onClick() {
				...
				if (previousVersion == 0 || SPDSettings.intro()){
					...
					SPDSettings.version(VoicedShatteredPixelDungeon.versionCode);
		...
		if (previousVersion != 0 && !SPDSettings.intro()){
			StyledButton changes = new StyledButton(Chrome.Type.GREY_BUTTON_TR, Messages.get(TitleScene.class, "changes")){
				...
				protected void onClick() {
					super.onClick();
					updateVersion(previousVersion);
					VoicedShatteredPixelDungeon.switchScene(ChangesScene.class);
				}
			};
		...
		} else if (previousVersion <= VoicedShatteredPixelDungeon.versionCode) {
